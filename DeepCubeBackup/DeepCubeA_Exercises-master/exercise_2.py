...
def approx_admissible_conv(env: Environment, nnet: nn.Module(), nnet_output: np.ndarray, optimal_output: np.ndarray, states: List[State], sample_states: np.ndarray, sample_outputs: np.ndarray, sample_optimal: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        while(complete_solve == False):
            ...
            h_new: Dict[State, float] = adjust_h_new(o_c_max, cutoffs, h_admissible, nnet_output, states, sample_states, sample_outputs)

            eta = 5.0
            count = 0
            for state in sample_states:
                if is_solved[state] is not True: 
                    h_admissible[state], is_solved[state] = a_star_update(env, nnet, state, h_new, h_admissible[state]+eta, states, nnet_output, o_c_max, cutoffs)
                count += 1
                print(count)
            ...
            plt.scatter(sample_optimal, np.array(list(h_admissible.values())), c = '000000', linewidths = 0.1)
            plt.axline([0,0],[30,30], linewidth =3, c = 'g')
            plt.ylabel('Converted output')
            plt.xlabel('Expected value')
            plt.title("Converted Output vs Expected")
        ...
        cutoffs: List = np.arange(0, int(max(sample_outputs))+1, 1).tolist()
        cutoffs.append(max(sample_outputs))
        o_c_max_final: List = get_o_c_max_final(cutoffs, nnet_output, sample_outputs, sample_states, h_admissible, states)    
        h_new: Dict[int, float] = adjust_h_new_final(o_c_max_final, cutoffs, h_admissible, nnet_output, states, sample_states, sample_outputs)
...
def h_prime(nnet_output: float, o_c_max: List[float], cutoffs: List[float]) -> float:
    #pdb.set_trace()
    maximum = np.argwhere(np.asarray(nnet_output) <= cutoffs).flatten()
    if len(maximum) > 0:
        return nnet_output - o_c_max[maximum[0]]
    else:
        return nnet_output - o_c_max[len(o_c_max)-1]

def adjust_h_new(o_c_max: List[float], cutoffs: List[float], h_admissible: Dict, nnet_output: np.ndarray, states: List, sample_states: np.ndarray, sample_outputs: np.ndarray) -> Dict[State, float]:
...
def adjust_h_new_final(o_c_max: List[float], cutoffs: List[float], h_admissible: Dict, nnet_output: np.ndarray, states: List, sample_states: np.ndarray, sample_outputs: np.ndarray) -> Dict[State, float]:
    #eq (4) h'(x) = h(x) - o_c_max
    h_new: Dict[int, float] = {hash(sample_states[i]): sample_outputs[i] - o_c_max[np.argwhere(sample_outputs[i] <= cutoffs).flatten()[0]] for i in range(len(sample_states))}
    #pdb.set_trace()
    return h_new
...
def a_star_update(env: Environment, nnet: nn.Module(), state: State, h_new: Dict[State, float], max_step: float, states: List[State], nnet_output: np.ndarray, o_c_max: List[float], cutoffs: List[float]) -> Tuple[float, bool]:
    update = AStarUpdate(env, nnet, state, h_new[hash(state)], max_step, states, nnet_output, o_c_max, cutoffs)
    lowest_cost_state = float('inf')
    first_node_removed: float = h_new[hash(state)]
    ...
    while len(update.opened) > 0:
        ...
        if update.env.is_solved([update.curr_state])[0] == True:
            pdb.set_trace()
            if update.opened[update.curr_state][1] < first_node_removed:
                return update.opened[update.curr_state][1], True
            else:
                if update.opened[update.curr_state][1] < lowest_cost_state:
                    lowest_cost_state = update.opened[update.curr_state][1]

        elif update.opened[update.curr_state][1] > max_step: 
            return update.opened[update.curr_state][0] + update.opened[update.curr_state][1], False   
        
        ''''
        if update.opened[update.curr_state][0] + update.opened[update.curr_state][1] < lowest_cost_state:
            lowest_cost_state = update.opened[update.curr_state][0] + update.opened[update.curr_state][1]
        '''
        update.delete()

    #Not sure if this should be considered solved or not  
    del update
    return lowest_cost_state, True



class AStarUpdate:

    def __init__(self, env: Environment, nnet: nn.Module(), state: State, h_new: float, max_step: float, states: List[State], nnet_output: np.ndarray, o_c_max: List[float], cutoffs: List[float]):
        self.env: Environment = env
        self.nnet: nn.Module() = nnet
        self.curr_state: State = state
        self.states: List[State] = states
        self.max_step: float = max_step
        self.o_c_max: List[float] = o_c_max
